...
function render(){
let q=$('search').value.toLowerCase();let modeFilter=$('filterMode').value;let typeFilter=$('filterType').value;let rows=data.filter(t=>(t.name+' '+t.location+' '+t.organizer+' '+(t.mode||'')+' '+(t.tournament_type||'')+' '+cat(t)+' '+t.format).toLowerCase().includes(q)&&(modeFilter==='all'||(t.mode||'Offline')===modeFilter)&&(typeFilter==='all'||(t.tournament_type||'Open')===typeFilter));
...
$('addBtn').onclick=()=>openTournament();$('addExpenseBtn').onclick=()=>openExpense();$('addIncomeBtn').onclick=()=>openIncome();$('close').onclick=close;$('cancel').onclick=close;$('search').oninput=render;$('filterMode').onchange=render;$('filterType').onchange=render; $('expenseSearch').oninput=render;$('incomeSearch').oninput=render;['registration','travel','food','accommodation','other','prizeInput'].forEach(id=>$(id).oninput=calc);
